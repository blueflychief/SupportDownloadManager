...
public class DownloadInfo {
    public static class Reader {
        ...
        public void updateFromDatabase(DownloadInfo info) {
            ...
            info.mCurrentSpeed = getLong(Downloads.COLUMN_CURRENT_SPEED);
            info.timeRemaining = getLong(Downloads.COLUMN_TIME_REMAINING);
    ...
    public boolean isReadyToDownload(long now) {
        if (mHasActiveThread) {
            // already running
            return false;
        }
    ...
    public boolean isDownloading(long now) {
        switch (mStatus) {
            case 0: // status hasn't been initialized yet, this is a new download
            case Downloads.STATUS_PENDING: // download is explicit marked as ready to start
            case Downloads.STATUS_RUNNING: // download interrupted (process killed etc) while
     ...
     * SupportDownloadManager.Request.NETWORK_* bit flag.
     */
    private int translateNetworkTypeToApiFlag(int networkType) {
        switch (networkType) {
            case ConnectivityManager.TYPE_MOBILE:
                return SupportDownloadManager.Request.NETWORK_MOBILE;

            case ConnectivityManager.TYPE_WIFI:
                return SupportDownloadManager.Request.NETWORK_WIFI;
