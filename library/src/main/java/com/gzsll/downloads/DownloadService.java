...
public class DownloadService extends Service {
            ...
            for (; ; ) {
                ...
                try {
                    ...
                    for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor
                        ...
                        boolean start = count < mSystemFacade.getMaxDownloadCount();
                        if (info != null) {
                            if (info.isDownloading(now)) {
                                updateDownload(reader, info, now, start);
                            } else {
                                updateDownload(reader, info, now, true);
                            }
                        } else {
                            info = insertDownload(reader, now, true);
                        }
     ...
     */
    private DownloadInfo insertDownload(DownloadInfo.Reader reader, long now, boolean start) {
    ...
    private void updateDownload(DownloadInfo.Reader reader, DownloadInfo info,
        ...
        if (start) {
            info.startIfReady(now);
        } else if (info.mStatus == Downloads.STATUS_RUNNING) {
